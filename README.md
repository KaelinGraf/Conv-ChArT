...
This repository is a from-scratch implementation: every architectural and numerical choice below is a deliberate design decision, documented inline with its reasoning, or a measurement quoted with its provenance.
...
The full design rationale — the Stage-3 decode chain, why the trunk is hybrid, the factorised heads and their loss, wavelength-anchored 2D axial RoPE, the gated-skip policy, and the deployment budget — lives in [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
└── docs/
    ├── ARCHITECTURE.md       full architecture description: decode chain, design rationale, loss, RoPE, gates, deployment budget
    ├── architecture.svg      the model diagram embedded above
...
| `scale_range_px: [12, 128]` | Sampling range for square size $s$ at input resolution, log-uniform (the training operating envelope). |
...
- `docs/ARCHITECTURE.md` — the full architecture description: Stage-3 decode chain, design rationale for the hybrid trunk, factorised heads and the penalty-reduced focal loss, wavelength-anchored RoPE, attention-gated skips, and the edge-deployment budget.
