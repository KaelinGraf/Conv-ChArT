...
**Two `DetectorNet` variants exist** (`attend_div`, see `dcc/model.py:DetectorNet`). The figure above is the native 5-stage shape (`attend_div=16`, 1600×1200 input) — the deployment target and the superset. Every checkpoint actually trained and ablated in this repo instead uses `attend_div=8` (640×480 input): `e1..e4` only (no `e5` — the dilated r=2,4 pair moves into `e4` as the now-deepest stage), attention over the H/8 grid (80×60 = 4 800 tokens, vs. the native 100×75 = 7 500), a single `gate3` (no `gate4`), and a `d3→d2→d1` decoder (no `d4`). Everything else — RoPE construction, gate/head shapes, block count, inits — is identical between the two.
...
**Why the hybrid trunk.** Convolutions ([Ronneberger2015]-style paired 3×3 blocks, with a dilated-convolution pair [Yu2016] added at the final encoder stage to cheaply widen its receptive field before the bottleneck) supply high-resolution, translation-equivariant features for localisation; the class head's identity read needs board-scale context (adjacent-marker radius ≈ 0.85$s$, full context propagation 1.85–2.85$s$, i.e. up to ≈ 365 px at $s=128$) that a conv-only stack cannot reach — its theoretical receptive field is ≈ 164 px and its *effective* RF is a Gaussian fraction of that ($\sigma \approx 31$ px, usable radius ~31–62 px) [Luo2016], further shrunk by skip dominance and ReLU. The two pre-norm transformer blocks [Vaswani2017] at the bottleneck are the closure — each block is two *sequential* residual sub-layers, $x \leftarrow x + W_O(\text{MHSA}(\text{LN}(x)))$ then $x \leftarrow x + \text{MLP}(\text{LN}(x))$, not a parallel pair (`dcc/model.py:Block.forward`) — after them, only three convolutions and one bilinear upsample separate attention output from class logits, none of which can destroy global content [Chen2021]. The decoder upsamples with bilinear interpolation + 3×3 conv at every stage, never a transposed convolution — stride-periodic checkerboard artifacts would beat directly against this system's own checkerboard target [Odena2016].
